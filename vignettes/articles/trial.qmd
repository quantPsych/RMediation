...
lavNames(fit)
lavInspect(fit0, what = "fit", add.labels = TRUE)
...
extract_table <- function(fit, conf.int = FALSE, conf.level = 0.95) {
    ...
    fit |>
        ...
        group_by(term) |>
        summarise(est = mean(estimate), var_b = var(estimate), var_w = mean(std.error^2), var_tot = var_w + var_b * (1 + 1 / length(res1@results)), std.error = sqrt(var_tot), p.value = exp(mean(log(p.value))))
