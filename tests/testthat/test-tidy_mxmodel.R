...
mx_model <- OpenMx::mxModel("SimpleModel",
  type = "RAM",
  manifestVars = manifestVars,
  latentVars = latVar,
  OpenMx::mxPath(from = "visual", to = c("x1", "x2", "x3"), values = 0.5, labels = "loadings"),
  OpenMx::mxPath(from = "textual", to = c("x4", "x5", "x6")),
  OpenMx::mxPath(from = "speed", to = c("x7", "x8", "x9")),
  OpenMx::mxPath(from = manifestVars, arrows = 2),
  OpenMx::mxPath(from = latVar, arrows = 2, free = FALSE, values = 1.0),
  OpenMx::mxPath(from = "one", to = manifestVars, arrows = 1, free = FALSE, values = 0),
  OpenMx::mxPath(from = "one", to = latVar, arrows = 1, free = FALSE, values = 0),
  OpenMx::mxData(hs_short, type = "raw"),
  OpenMx::mxCI(c("A", "S", "M")) # Add confidence intervals
)
...
test_that("tidy.MxModel returns correct structure", {
  ...
  expect_true(all(c("conf_low", "conf_high") %in% names(result_with_ci)))
})
